...
impl<'a> TerminalView<'a> {
    ...
    fn process_input(self, layout: &Response, state: &mut TerminalViewState) -> Self {
        ...
        for event in events {
            if event_requires_pointer(&event) && !contains_pointer {
                continue;
            }
...
mod input_tests {
    ...
    fn keyboard_is_focus_gated_but_not_pointer_gated() {
        assert!(!event_requires_pointer(&egui::Event::Text("hello".into())));
        assert!(!event_requires_pointer(&egui::Event::Paste("hello".into())));
        assert!(event_requires_pointer(&egui::Event::PointerMoved(
            egui::pos2(1.0, 1.0)
        )));
        assert!(event_requires_pointer(&egui::Event::MouseWheel {
