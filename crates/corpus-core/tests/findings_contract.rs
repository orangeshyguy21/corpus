...
fn production_projection_matches_the_contract() {
    ...
    let missing = corpus_core::finding_cards(&store, "missing")
        .unwrap_err()
        .to_string();
...
fn writer_serializes_reserved_and_extension_metadata_safely() {
    ...
    let (frontmatter, body) = corpus_core::frontmatter::split(&raw).unwrap();
    ...
    assert_eq!(
        corpus_core::frontmatter::get_str(&frontmatter, "sensitivity").as_deref(),
        Some("embargoed")
    );
    assert_eq!(
...
fn writer_is_collision_safe_and_confines_agent_chosen_paths() {
    ...
    .unwrap();
    assert_eq!(
        corpus_core::read_finding(&store, "project", "findings/custom/exact.md")
            .unwrap()
            .as_bytes(),
        before
    );
    let error = corpus_core::read_finding(&store, "project", "techniques/not-a-finding.md")
        .unwrap_err()
        .to_string();
