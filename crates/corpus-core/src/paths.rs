...
fn resolve_resource_root() -> std::result::Result<PathBuf, String> {
    ...
    let check = |dir: PathBuf, tried: &mut Vec<String>| -> Option<PathBuf> {
        ...
        tried.push(format!("{}{}", dir.display(), if ok { " (ok)" } else { "" }));
        ok.then_some(dir)
    };
...
pub fn resources_for_plugins() -> Result<PathBuf> {
    if let Some(dir) = std::env::var(crate::registry::PLUGINS_DIR_ENV)
        .ok()
        .filter(|s| !s.is_empty())
    {
...
pub fn corpus_mcp_bin() -> Result<PathBuf> {
    if let Some(explicit) = std::env::var("CORPUS_MCP").ok().filter(|s| !s.is_empty()) {
        return Ok(PathBuf::from(explicit));
    }
    let mut tried: Vec<String> = Vec::new();
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            for candidate in [dir.join("corpus-mcp"), dir.join("../corpus-mcp")] {
    ...
    if let Some(root) = resource_root_opt() {
        for candidate in [
            root.join("bin/corpus-mcp"),
            root.join("target/debug/corpus-mcp"),
        ] {
            if candidate.is_file() {
                return Ok(candidate);
            }
    ...
    Err(Error::Store(format!(
        "corpus-mcp binary not found — tried: {}. Set CORPUS_MCP to its path.",
        tried.join(", ")
    )))
}
...
mod tests {
    ...
    fn an_explicit_resource_root_must_be_real() {
        ...
        assert!(is_resource_root(&PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..")));
