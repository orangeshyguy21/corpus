...
fn pull_model_list(refresh: bool) -> Result<ModelList, Error> {
    let opencode = crate::launch::resolve_opencode()?;
...
fn parse_verbose_models(text: &str) -> Vec<(String, String)> {
    ...
    for line in text.lines() {
        let record_start = !line.is_empty()
            && !line.starts_with(char::is_whitespace)
            && line.contains('/');
        if record_start {
            if let Some((id, name)) = current.take() {
                out.push((id.clone(), name.unwrap_or_else(|| model_part(&id).to_string())));
            }
...
"#;
    ...
    fn verbose_parse_extracts_ids_and_real_names() {
        ...
        assert_eq!(
            entries[1],
            ("ollama/qwen3.6:35b".to_string(), "Qwen3.6 35B (MoE)".to_string())
        );
    ...
    fn grouping_sorts_providers_and_models_and_labels_known() {
        ...
        assert_eq!(list.groups[0].label, "mystery", "unknown provider keeps the raw id");
    ...
    fn live_model_list_groups_real_providers() {
        ...
        assert!(list.groups.iter().flat_map(|g| g.models.iter()).all(|m| !m.name.is_empty()));
    }
}
